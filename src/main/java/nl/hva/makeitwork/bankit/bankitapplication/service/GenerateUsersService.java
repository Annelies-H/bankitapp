...
- Surname contains both the lastName and the prefix(es)
...
/**
    ...
    private Person parsePerson(String[] row) {
        Calendar birthday = parseBirthday(row[BIRTHDAY_INDEX]);
        String[] lastNameSplit = row[LASTNAME_PREFIX_INDEX].split(" ");
        ...
        String initials = row[FIRSTNAME_INDEX].charAt(0) + row[MIDDLENAME_INITIAL_INDEX];
        Person person = new Person(row[FIRSTNAME_INDEX], prefix, lastName, initials, birthday, row[GENDER_INDEX]);
        return person;
    }

    private Calendar parseBirthday(String date) {
    ...
    private ContactDetails parseContactDetails(String[] row) {
        Address address = parseAddress(row);
        String phone = row[PHONE_INDEX].replace("-", "");
        ContactDetails contact = new ContactDetails(address, row[EMAIL_INDEX], phone);
        return contact;
    }

    private Address parseAddress(String[] row) {
        String[] splitStreet = row[STREET_NR_INDEX].split(" ");
        ...
        String zipcode = row[ZIPCODE_INDEX].replace(" ", "");
        Address address = new Address(streetName, houseNr, null, zipcode, row[CITY_INDEX], row[COUNTRY_INDEX]);
        return address;
    }
